...
class AnnoyBuildIndex(index.AnnBuildIndex):
    ...
    def build(self):
        ...
        self._index.build(n_trees=ntrees)
        self._index.unload()
...
class AnnoyQueryIndex(index.AnnQueryIndex):
    ...
    def query(
        ...
        res = self._index.get_nns_by_vector(
            qemb.tolist(),  # pyright: ignore
            self._cfg.max_results_per_query,
            search_k=self._cfg.search_k,
            include_distances=self._cfg.include_distances,
        )
        if self._cfg.include_distances:
            indices, distances = res
        else:
            indices = res
